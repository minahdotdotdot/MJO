...
#TODO: eliminate or generalize type parameter, which is a no-op here.
@inline function partial_y{T<:Number}(q:: Array{T, 2}, ii::Int64, jj::Int64, delt_y::T)
    value = 0

    #TODO: is it possible to eliminate these branching points in this inner loop?
    if jj == 1
        ### LOWER BOUNDARY ###
        value = 1./delt_y*(q[jj+1,ii]-q[jj,ii])

    elseif jj == size(q)[1]
        ### UPPER BOUNDARY ###
        value = 1./delt_y*(q[jj,ii]-q[jj-1,ii])

    else
        ### INTERIOR ###
        value = .5/delt_y*(q[jj+1,ii]-q[jj-1,ii])
    end
    return value
end
...
function dxdt(params:: MJO_params, state::MJO_State, out::MJO_State)
	...
	#TODO(minah): it's ok to make this an elif, no?
        if ii == 1440
            iiii = 0 #to satisfy iiii+1 = 1
        end
        #println("column: ",ii)
        ...
        for jj = 2: length(state.y) -1
            ...
            - params.Fr*( state.h1[jj,ii] * partial_y(h_sum, ii, jj, delt_y)
                ) #=h_1∂y(h_1+h_2)=# 
	    ...
	    #TODO(gregor): identify why this term is costly (due to *,+ broadcast).
            out.n2[jj,ii] =
            + div_flux(state.m2, state.n2, state.h2, state.n2, ii, jj, delt_x, delt_y)
            - params.Ro*state.y[jj]*state.m2[jj,ii] #=-1/Ro*m2=#
            - params.Fr*( state.h1[jj,ii] * partial_y(h_sum_a, ii, jj, delt_y)
                ) #=h_1∂y(h_1+\alpha*h_2)=#
            + state.n1[jj,ii]/state.h1[jj,ii]*value_P_RC 
            #println("n2 done.")
            ...
            - partial_y(state.n1, ii, jj, delt_y) #=∂y n_1=#
            - value_P_RC
            #println("h1 done.")

            out.h2[jj,ii] =
            -.5/delt_x * (state.m2[jj,iiii+1] - state.m2[jj,iii-1]) #=∂x m_2=#
            - partial_y(state.n2, ii, jj, delt_y) #=∂y n_2=#
            + value_P_RC
            #println("h2 done.")
...
state = MJO_State(.25*pi/180*params.RE/params.LL,
          ...
          linspace(-20.+.125, 20.-.125, grid_y),
          pi/180*params.RE * linspace(-20.+.125, 20.-.125, grid_y)/params.LL,
          zeros(grid_y, grid_x),
          zeros(grid_y, grid_x),
          zeros(grid_y, grid_x),
          zeros(grid_y, grid_x),
          ...
          zeros(grid_y, grid_x));
out = deepcopy(state)
#out = MJO_State(.25*pi/180*params.RE/params.LL, .25*pi/180*params.RE/params.LL, linspace(0., 360.-.25, grid_x), linspace(-20.+.125, 20.-.125, grid_y),
#    pi/180*params.RE * linspace(-20.+.125, 20.-.125, grid_y)/params.LL,
#    zeros(grid_y, grid_x), zeros(grid_y, grid_x), zeros(grid_y, grid_x), zeros(grid_y, grid_x), ones(grid_y, grid_x), ones(grid_y, grid_x), zeros(grid_y, grid_x));
