...
They use $\epsilon = \hat{\beta}Q/Q_s$ where $Q_s$ is a saturation level and $\hat{\beta} = 1.1$.
...
\end{equation}
where $H$ is the heaviside function.{\color{blue}$\frac{c_p}{L}$ has the units of temperature.} The heaviside function ensure that only radiative cooling (and not heating!) occurs. 
...
This $a(t)$ is used to enforce a constant total amount of precipitation over the area at each time-step. 
